...
// Read lock times from chain instead of envs:
const LIQ_ADDRESS     = (process.env.NEXT_PUBLIC_LIQ_ADDRESS || "0x793ec280d1fd72ecabedfd984f11a39a50e1c5b7").trim() as `0x${string}`;
const TEAMLOCK_ADDR   = (process.env.NEXT_PUBLIC_TEAM_LOCK_ADDRESS || "0xbeA426A7D608F746Ed843a8EeAF5c09BA971A793").trim() as `0x${string}`;
...
const LIQ_ABI = parseAbi([
  "function lpLockedUntil() view returns (uint256)",
] as const);

const TEAMLOCK_ABI = parseAbi([
  "function releaseAt() view returns (uint256)",
] as const);
...
const tsLocale = Intl.DateTimeFormat().resolvedOptions().timeZone || "local";
...
const RIM = "#2a2f3a";
const PIE_COLORS = ["#BBA46A", "#6C5E3A", "#3E3A2E", "#222428"]; // tuned for contrast
...
export default function LaunchPage() {
  ...
  /** Reads - Locks (from chain) */
  const { data: lpUntil }   = useReadContract({ address: LIQ_ADDRESS,   abi: LIQ_ABI,      functionName: "lpLockedUntil" });
  const { data: teamUntil } = useReadContract({ address: TEAMLOCK_ADDR, abi: TEAMLOCK_ABI, functionName: "releaseAt" });
    ...
    <main className="min-h-screen bg-[#0b0e14] text-zinc-100">
      ...
      <div className="border-b border-zinc-800/60 bg-zinc-900/20 backdrop-blur">
        <div className="mx-auto max-w-6xl px-6 py-3 flex items-center justify-between">
          ...
          <div className="text-xs md:text-sm text-zinc-500">
            {PRESALE_ADDRESS ? (
              <>Presale: <span className="text-[#BBA46A] font-medium">{PRESALE_ADDRESS.slice(0,6)}…{PRESALE_ADDRESS.slice(-4)}</span></>
            ) : (
      ...
      <section className="mx-auto max-w-6xl px-6 py-10">
        <div className="grid gap-8 lg:grid-cols-[380px,1fr]">
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-zinc-800/70 bg-zinc-900/40 p-6">
              ...
              <p className="mt-2 text-sm text-zinc-400">
                Fixed-price presale on <span className="text-[#BBA46A]">Base</span>. Unsold tokens seed
                liquidity on Aerodrome. Team allocation locked. Holder rewards via a separate
                <span className="text-[#BBA46A]"> 500M-tiered program</span> (see link below).
              </p>
              ...
              {/* Progress */}
              <div className="mt-6">
                <div className="flex items-center justify-between text-[11px] sm:text-xs text-zinc-400 tracking-wide uppercase">
                  <span>Raised</span>
                  <span>
            ...
            <div className="rounded-2xl border border-zinc-800/70 bg-zinc-900/40 p-6">
              <h3 className="text-lg font-semibold text-[#BBA46A] tracking-wide">Token Distribution</h3>
              <div className="mt-4 h-56 sm:h-64 md:h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    >
                      {distData.map((_, idx) => (
                        <Cell key={idx} fill={PIE_COLORS[idx % PIE_COLORS.length]} stroke="#e5e7eb22" />
                      ))}
                    </Pie>
                    <Tooltip
                      contentStyle={{
                    ...
                    <Legend
                      ...
                      formatter={(value: string) => (
                        <span className="text-zinc-300">
                          <span className="text-[#BBA46A] font-medium">{value}</span>
                        </span>
                      )}
                    />
                  </PieChart>
              ...
              <p className="mt-4 text-[11px] sm:text-xs leading-relaxed text-zinc-400">
                * An additional <span className="text-[#BBA46A] font-medium">500,000,000</span> tokens are
                reserved in the token contract and released to holders in{" "}
                <span className="text-[#BBA46A]">tiered schedules</span> based on holding duration.
                See details here:{" "}
          ...
          <div className="space-y-6">
              ...
              <div className="mt-4 overflow-hidden rounded-xl border border-zinc-800/60">
                <table className="w-full text-sm">
                  <tbody className="divide-y divide-zinc-800/60">
                    ...
                    <Row k="LP lock until"   v={`${ts(lpUntil as bigint)}  ·  ${rel(lpUntil as bigint)}`} />
                    <Row k="Team lock until" v={`${ts(teamUntil as bigint)}  ·  ${rel(teamUntil as bigint)}`} />
                  </tbody>
                </table>
              </div>
            ...
            {/* Contracts & Links (above disclaimer) */}
            <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/40 p-6">
      ...
      <footer className="border-t border-zinc-800/60 bg-zinc-900/20">
        <div className="mx-auto max-w-6xl px-6 py-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="text-xs text-zinc-500">Built for Base · Styled with gold accents</div>
        </div>
      </footer>
    </main>
  );
