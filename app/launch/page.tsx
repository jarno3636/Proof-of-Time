...
// We’ll try both common names for claim unlock:
      ...
      <div className="border-b border-zinc-800/60 bg-zinc-900/20 backdrop-blur">
        <div className="mx-auto max-w-6xl px-6 py-3 flex items-center justify-between">
          ...
          <div className="text-xs md:text-sm text-zinc-500">
            {PRESALE_ADDRESS ? <>Presale: <span className="text-[#BBA46A] font-medium">{PRESALE_ADDRESS.slice(0,6)}…{PRESALE_ADDRESS.slice(-4)}</span></> : "Set NEXT_PUBLIC_PRESALE_ADDRESS"}
          </div>
        </div>
      </div>
            ...
            <div className="rounded-2xl border border-zinc-800/70 bg-zinc-900/40 p-6">
              <div className="flex items-center justify-between gap-3">
                ...
                {saleEndUnix ? (
                  <div className="text-xs text-zinc-400">
                    Ends in <span className="text-[#BBA46A] font-semibold"><Countdown target={saleEndUnix} /></span>
                  </div>
                ) : null}
              </div>
              ...
              <div className="mt-5 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <button
                    onClick={onBuy} disabled={!canBuy || isPending || confirming}
                ...
                {isConnected && chainId !== base.id && <div className="text-xs text-red-400">Switch to Base to continue.</div>}
              ...
              <div className="mt-6">
                <div className="flex items-center justify-between text-[11px] sm:text-xs text-zinc-400 uppercase tracking-wide">
                  <span>Raised</span>
                  <span>{progress.r.toLocaleString()} / {progress.h.toLocaleString()} ETH ({progress.pct.toFixed(1)}%)</span>
                </div>
            ...
            <div className="rounded-2xl border border-zinc-800/70 bg-zinc-900/40 p-6">
              ...
              <div className="mt-4 h-56 sm:h-64 md:h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      dataKey="value" data={distData} cx="50%" cy="46%"
                      outerRadius={94} innerRadius={52}
                      labelLine={false} paddingAngle={1.2}
                    >
                      {distData.map((_, i) => (
                        <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} stroke="#00000055" />
                      ))}
                    </Pie>
                    <Tooltip contentStyle={{ background: "#0f1116", border: "1px solid #2a2f3a", borderRadius: 12, color: "#e5e7eb" }} />
                    <Legend
                      verticalAlign="bottom" align="center" iconType="square"
              ...
              <p className="mt-4 text-[11px] sm:text-xs leading-relaxed text-zinc-400">
                * An additional <span className="text-[#BBA46A] font-medium">500,000,000</span> tokens are reserved in the token contract
                ...
                <Link href={POT_LINK} className="underline text-[#BBA46A] hover:text-[#d6c289]">Holder Rewards (PoT)</Link>.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/40 p-6">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3 text-sm">
                {PRESALE_ADDRESS && (
                  <a href={`https://basescan.org/address/${PRESALE_ADDRESS}`} target="_blank" rel="noopener noreferrer"
                ...
                {TOKEN_ADDRESS && (
                  <a href={`https://basescan.org/address/${TOKEN_ADDRESS}`} target="_blank" rel="noopener noreferrer"
                     className="inline-flex items-center gap-2 rounded-lg border border-zinc-800/70 bg-zinc-900/40 px-3 py-1.5 font-semibold text-[#BBA46A] hover:text-[#d6c289] hover:border-[#BBA46A]/50 transition">
                    View Token ↗
                  </a>
                )}
...
function A({ href, label }: { href: string; label: string }) {
  ...
  return isExternal ? (
    <a href={href} target="_blank" rel="noopener noreferrer" className={classes}>{label} ↗</a>
  ) : (
